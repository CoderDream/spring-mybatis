...
var communityOperate=function(){
	...
	selectEditClickInBuffer:function(response){
    	...
    	require([ "esri/map",
           ...
           editToolbar_communtity.activate(esri.toolbars.Edit.EDIT_VERTICES , selected_graphics);
    	  });
...
//			"&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:;' class='infowindow_a'  onClick='dataOperate.updateBuildingDeal("+evt.graphic.attributes.ID+")'>确认匹配</a>
		//散楼显示模板
		getBuild:function(evt){
			   ...
			   var content="<table id='"+evt.graphic.attributes.ID+"_table'><tr><td style='width: 60px;'>地址全称：</td><td>"+mainApp.getValue(evt.graphic.attributes.NAME)+"</td></tr>" +
		   		...
		   		"<tr><td>街道号：</td><td><input type='text' id='"+evt.graphic.attributes.ID+"_c' value='"+mainApp.getValue(evt.graphic.attributes.PARAMOBJ.data.ROADNO)+"'></td></tr>" +
			    "<tr><td>楼栋名称：</td><td><input type='text' id='"+evt.graphic.attributes.ID+"_a'  value='"+mainApp.getValue(evt.graphic.attributes.PARAMOBJ.data.BUILDINGNAME)+"'></td></tr> ";
			   //如果审核通过不在显示保存按钮
			   //alert(evt.graphic.attributes.PARAMOBJ.data.ISVERIFY);
		   		if(evt.graphic.attributes.PARAMOBJ.data.ISVERIFY==0){
		   			content+= "<tr><td colspan='2'><a href='javascript:;' class='infowindow_a'  onClick='dataOperate.updateAddress("+evt.graphic.attributes.ID+")'>保存地址</a></td></tr>" ;
				}
		   		content+="</table>";
			   return content;
		}
