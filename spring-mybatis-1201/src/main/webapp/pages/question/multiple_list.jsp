...
<body class="main_body">
  <div class="button_bar">
		<button class="button_b_b button_bar_btnText" onclick="to('${basePath}secure/forward.action?target=multipleInput');">继续新增</button>
...
<form id="searchForm" action="secure/multipleQuestion!findByPage.action" method="post">
	<!-- 试题类型 -->
  	<s:hidden name="questType" value="2"/>
 ...
 <table border="0" width="100%">
  <tr>
	...
	<td>
	<fieldset>
		<legend>考卷出多选题查询</legend>
		<div id="searchDiv" style="margin-left:210px;">
		   	<span>
			   	<label><s:text name="title.multiple.body"/></label>
  ...
  <tr>
  	<td style="border: 1px solid gray;">
  		<!-- 列表 -->
  		<div class="listDiv">
		  <s:iterator value="multiples" status="stat">
		  	<div id="question<s:property value="id"/>" class="radio_list">
			  	...
			  	<div id='qo_<s:property value="#stat.count"/>' class="radio_list_options">
				  	<ul style="margin-left:7px;">
				  		...
				  		<s:if test="@com.olts.commons.StringTools@isNotEmpty(selectOption.optionE)">
				  	...
				  	<span style="margin-left:20px;">
				  		<input type="button" value="更新" 
				  			onclick="location.href='secure/multipleQuestion!findById.action?id=<s:property value="id"/>&catagoryId=<s:property value="questCatagory.catagory.id"/>';"/>
				  		<input type="button" value="删除" onclick='doDelete(<s:property value="id"/>,<s:property value="selectOption.id"/>);' style="margin-right:20px;"/>
				  	</span>
			  	</div>
		  	</div>
		  </s:iterator>
		  	<s:if test="multiples!=null && !multiples.isEmpty()">
		  	<div class="add_exam">
		  		...
		  		<label><font color="red"><b>*</b></font><s:text name="title.examno"/></label>
		  		...
		  		<font color="red"><s:text name="prompt.examno"/></font><br />
		  		...
		  		<s:textfield name="examination.examDate" size="8" value="%{#session.ExamDate}" title="请选择考试日期"
		  		 ...
		  		 -->
		  		<input type="button" name="addExam" class="btn_b_b button_bar_btnText" value="加入多选题" title="加入多选题到考卷" style="margin-left:30px;margin-top:5px;"/>
